...
def calc_stats(file_lines: [str]) -> ([], [], [], [], int):
	""" Takes lines of .stats file as list
	Returns Tuple with (esil_only, rzil_only, both, none, sum_instructions)
	The lists in the tubple hold dicts {'name': Insn_enum_name, 'count': how often it was seen.}
	"""
	sum_insn = 0
	esil_only = list()
	rzil_only = list()
	...
	for l in file_lines:
		...
		if e == 'e' and r == 'r':
			...
			esil_only.append({'name': insns_ids(insn).name, 'count': count})
		elif e == 'x' and r == 'r':
			rzil_only.append({'name': insns_ids(insn).name, 'count': count})
		elif e == 'x' and r == 'x':
			none.append({'name': insns_ids(insn).name, 'count': count})
		else:
			print(f'Incorrect formatted: e = {e} r = {r}')
		sum_insn += count
	return (esil_only, rzil_only, both, none, sum_insn)

def print_stats(stat_file):
	sum_insn = 0

	with open(stat_file) as f:
		esil_only, rzil_only, both, none, sum_insn = calc_stats(f.readlines())
